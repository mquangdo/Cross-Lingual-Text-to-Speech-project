    ...
    def ssim(self, est, target):
        ...
        for bs in range(output.shape[0]):
            for c in range(output.shape[1]):
                res[bs, c] = ssim(output[bs, c, ...], target[bs, c, ...], win_size=7, data_range=target[bs, c, ...].max() - target[bs, c, ...].min())
...
if __name__ == "__main__":
    ...
    # path1 = "old/out.wav"
    path1 = "infer_tacotron-hifigan/data0/350.wav"
    # path2 = "old/target.wav"
    path2 = "test_wavs_4-5s/350.wav"
    result = au.evaluation(path2, path1)
    print(result)