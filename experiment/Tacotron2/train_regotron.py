...
from regotron_loss import regotron_monotonic_alignment_loss
...
def parse_args():
    ...
    # === Regotron Monotonic Alignment Loss ===
    # Thay thế guided_attention_loss từ DC-TTS.
    # Paper khuyến nghị: lambda=1e-5, delta=0.01
    # Không cần warmup vì loss này tự nhiên = 0 khi alignment đã monotonic.
    parser.add_argument("--use_regotron_loss", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--regotron_lambda", type=float, default=1e-5)  # λ trong paper
    parser.add_argument("--regotron_delta", type=float, default=0.01)   # δ trong paper
...
def _ensure_attn_B_Tdec_Tenc(attn: torch.Tensor) -> torch.Tensor:
    """Normalize attention shape về [B, T_dec, T_enc]."""
    if attn is None or attn.dim() != 3:
        return None
    B, A, C = attn.shape
    # T_enc (chars) thường nhỏ hơn T_dec (mel frames)
    if A < C:
...
def main():
    ...
    trainset = TTSDataset(args.path_to_train_manifest, sample_rate=args.sampling_rate, n_fft=args.n_fft,
                          window_size=args.window_size, hop_size=args.hop_size, fmin=args.fmin, fmax=args.fmax,
                          num_mels=args.num_mels, min_db=args.min_db, max_scaled_abs=args.max_scaled_abs)

    testset = TTSDataset(args.path_to_val_manifest, sample_rate=args.sampling_rate, n_fft=args.n_fft,
                         window_size=args.window_size, hop_size=args.hop_size, fmin=args.fmin, fmax=args.fmax,
                         num_mels=args.num_mels, min_db=args.min_db, max_scaled_abs=args.max_scaled_abs)
    ...
    for epoch in range(completed_epochs, args.training_epochs):
        ...
        for texts, text_lens, mels, stops, encoder_mask, decoder_mask in trainloader:
            ...
            loss = mel_loss + refined_mel_loss * args.refined_mel_weight + stop_loss * args.stop_weight

            # === Regotron Monotonic Alignment Loss ===
            # Không cần warmup — loss tự = 0 khi alignment đã đúng.
            # Không có computational overhead thêm (chỉ tính trên attention matrix).
            reg_loss = torch.zeros(1, device=accelerator.device)
            if args.use_regotron_loss:
                attn = _ensure_attn_B_Tdec_Tenc(attention_weights)
                if attn is not None:
                    reg_loss = regotron_monotonic_alignment_loss(
                        attention_weights=attn,
                        text_lens=text_lens.to(attn.device),
                        mel_lens=mel_lens.to(attn.device),
                        delta=args.regotron_delta,
                    )
                    loss = loss + args.regotron_lambda * reg_loss
            ...
            reg_item     = torch.mean(accelerator.gather_for_metrics(reg_loss)).item()

            if completed_steps % args.console_out_iters == 0:
                accelerator.print(
                    "Steps {}/{} | Loss {:.4f} | Mel {:.4f} | RMel {:.4f} | Stop {:.4f} | Regotron {:.6f}".format(
                        completed_steps, args.training_epochs * len(trainloader),
                        loss_item, mel_item, rmel_item, stop_item, reg_item,
                    )
                )

            if completed_steps % args.wandb_log_iters == 0 and args.log_wandb:
                accelerator.log(
                    {"mel_loss": mel_item, "refined_mel_loss": rmel_item,
                     "stop_loss": stop_item, "regotron_loss": reg_item,
        ...
        val_mel_loss, val_rmel_loss, val_stop_loss, val_reg_loss = 0, 0, 0, 0
        num_losses = 0
        save_first = True
        ...
        for texts, text_lens, mels, stops, encoder_mask, decoder_mask in testloader:
            ...
            # Regotron loss trên validation (chỉ để monitor, không backprop)
            reg_loss = torch.zeros(1, device=accelerator.device)
            if args.use_regotron_loss:
                attn = _ensure_attn_B_Tdec_Tenc(attention_weights)
                if attn is not None:
        ...
        val_reg  = torch.mean(accelerator.gather_for_metrics(val_reg_loss)).item()  / num_losses
        val_total = val_mel + val_rmel + val_stop

        accelerator.print(
            "Loss {:.4f} | Mel {:.4f} | RMel {:.4f} | Stop {:.4f} | Regotron {:.6f}".format(
                val_total, val_mel, val_rmel, val_stop, val_reg
            )
        )

        if args.log_wandb:
            accelerator.log(
                {"val_mel_loss": val_mel, "val_refined_mel_loss": val_rmel,
                 "val_stop_loss": val_stop, "val_regotron_loss": val_reg,
