...
class Prenet(nn.Module):
    ...
    def forward(self, x):
        for layer in self.layers:
            ...
            # x = F.dropout(layer(x), p=self.dropout_p, training=False) # cuonglp1
...
class Decoder(nn.Module):
    ...
    def inference(self, encoder_output, max_decode_steps=1000):
        device = encoder_output.device # cuonglp1
        start_feature_vector = self._bos_frame(B=1).squeeze(0).to(device)
